...
public class ModProfileActivity extends AppCompatActivity {
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == CAMERA_REQUEST && resultCode == RESULT_OK) {
      ...
      try {
        ...
        //파일 업로드 경로 및 파일네임(DB저장용)
        imageDbPathU = ipConfig + pServer + member_imgPath + uploadFileName;

        Glide.with(ModProfileActivity.this).load(imageRealPathU)
            .error(R.drawable.user).into(mod_faceImg);
  ...
  public void mod_saveBtnClicked(View view){
    if(isNetworkConnected(this) == true){
      //단 비번이 입력되어 있다면 두 값을 비교해서 같으면 저장, 아니면 다르다고 메시지 띄운다
      //수정 시 확인 메시지 띄움
